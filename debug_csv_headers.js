#!/usr/bin/env node
// debug_csv_headers.js — Mostra os cabeçalhos do CSV e tenta mapear colunas do motorista

const fs = require('fs');
const path = require('path');

// ─── MUDA AQUI para o caminho do teu CSV ───────────────────────────────────
// Exemplo: const CSV_PATH = 'C:/Users/ThiagoSousa/Downloads/20260413-20260420-payments_driver-DCADA_OUSADA_LDA.csv';
const CSV_PATH = process.argv[2] || null;
// ────────────────────────────────────────────────────────────────────────────

if (!CSV_PATH) {
  console.error('Uso: node debug_csv_headers.js "C:/caminho/para/ficheiro.csv"');
  process.exit(1);
}
...
// Aliases que o sistema usa para motorista
...
for (const alias of DRIVER_ALIASES) {
  ...
  if (idx >= 0) {
    console.log(`  ✅ Encontrada coluna de motorista: "${headers[idx]}" (índice ${idx})`);
    found = true;
  }
}
if (!found) {
  console.log('  ❌ Nenhuma coluna de motorista reconhecida!');
  console.log('\n  Colunas que contêm "driver" ou "motorista":');
  headers.forEach((h, i) => {
