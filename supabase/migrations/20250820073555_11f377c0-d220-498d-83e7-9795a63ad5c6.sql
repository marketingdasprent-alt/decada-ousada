-- Allow admins to be auto-assigned on first view in addition to Gestor TVDE users
...
BEGIN
  ...
  SELECT nome, cargo, is_admin INTO user_name, user_cargo, user_is_admin 
  FROM profiles 
  WHERE id = user_id_param;
  
  -- Se não encontrou o usuário/nome ou não é gestor nem admin, retornar false
  IF user_name IS NULL OR NOT (COALESCE(user_is_admin, false) = TRUE OR user_cargo = 'Gestor TVDE') THEN
    RETURN FALSE;
  END IF;
  ...
  -- Atribuir o lead ao usuário (admin ou gestor) que visualizou primeiro
