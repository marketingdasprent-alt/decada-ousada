...
export const ReservaTabCondutores: React.FC<ReservaTabCondutoresProps> = ({ form, clientes }) => {
  ...
  // Clientes que ainda não estão na lista de condutores
  const clientesDisponiveis = useMemo(() => {
    const usados = new Set(fields.map((f) => f.cliente_id));
    return clientes.filter((c) => !usados.has(c.id));
  }, [clientes, fields]);

  const handleAdicionarCondutor = (newClienteId: string) => {
  ...
  const handleAdicionarClienteDaReserva = () => {
    if (!cliente) return;
    handleAdicionarCondutor(cliente.id);
  };
  ...
  const handleRemover = (idx: number) => {
    ...
    if (removendoPrincipal) {
      // Marcar o primeiro restante como principal automaticamente
      setTimeout(() => {
  ...
  const clienteDaReservaJaEhCondutor = cliente
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  <CommandGroup>
                    {clientesDisponiveis.map((c) => (
                      <CommandItem
                        ...
                        onSelect={() => handleAdicionarCondutor(c.id)}
          ...
          {cliente && !clienteDaReservaJaEhCondutor && (
            <Button
              type="button"
              size="sm"
              variant="outline"
              onClick={handleAdicionarClienteDaReserva}
              className="gap-2"
            >
              <UserCheck className="h-4 w-4" />
              Cliente da Reserva também conduz
            </Button>
          )}
        </div>
