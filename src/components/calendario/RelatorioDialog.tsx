...
const TIPOS_CONFIG = [
  { value: 'entrega',      label: 'Entrega',           color: 'border-green-500 bg-green-500/10 text-green-700 dark:text-green-400',    colorActive: 'border-green-500 bg-green-500 text-white' },
  { value: 'recolha',      label: 'Recolha',           color: 'border-blue-500 bg-blue-500/10 text-blue-700 dark:text-blue-400',        colorActive: 'border-blue-500 bg-blue-500 text-white' },
  { value: 'devolucao',    label: 'Devolução',         color: 'border-orange-500 bg-orange-500/10 text-orange-700 dark:text-orange-400', colorActive: 'border-orange-500 bg-orange-500 text-white' },
  { value: 'troca',        label: 'Troca',             color: 'border-purple-500 bg-purple-500/10 text-purple-700 dark:text-purple-400', colorActive: 'border-purple-500 bg-purple-500 text-white' },
  { value: 'upgrade',      label: 'Upgrade',           color: 'border-yellow-500 bg-yellow-500/10 text-yellow-700 dark:text-yellow-400', colorActive: 'border-yellow-500 bg-yellow-500 text-white' },
  { value: 'lista_espera', label: 'Lista de Espera',   color: 'border-pink-500 bg-pink-500/10 text-pink-700 dark:text-pink-400',         colorActive: 'border-pink-500 bg-pink-500 text-white' },
];

const TIPO_LABELS: Record<string, string> = Object.fromEntries(TIPOS_CONFIG.map(t => [t.value, t.label]));
...
export const RelatorioDialog: React.FC<Props> = ({ open, onOpenChange, currentMonth }) => {
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      doc.text(`${eventosFiltrados.length} evento(s)`, pageW - marginR, statsY + 8.5, { align: 'right' });
      ...
      eventosFiltrados.forEach((ev, i) => {
        ...
        doc.setFontSize(9.5);
        doc.setTextColor(20, 20, 25);
        const matricula = ev.tipo === 'lista_espera'
          ? ev.titulo
          : ev.tipo === 'troca'
      ...
      if (totais.length > 0) {
        ...
        const d1 = format(new Date(dataInicio + 'T00:00:00'), 'dd/MM/yyyy');
        const d2 = format(new Date(dataFim + 'T00:00:00'), 'dd/MM/yyyy');
        doc.text(`Período: ${d1} — ${d2}   ·   ${eventosFiltrados.length} evento(s) no total`, marginL, gy);
        gy += 12;
        ...
        totais.forEach((t, i) => {
          const pct = eventosFiltrados.length > 0
  ...
  const exportarPDFPorGestor = async () => {
    ...
    try {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(99, 102, 241);
      doc.text(`${eventosFiltrados.length} evento(s)  ·  ${gestores.length} gestor(es)`, pageW - marginR, headerH + 8.5, { align: 'right' });
      ...
      for (const gestor of gestores) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8);
        doc.text(`${gestor.eventos.length} evento(s)`, pageW - marginR, y + 7.5, { align: 'right' });
        y += sectionH;
        ...
        gestor.eventos.forEach((ev, i) => {
          ...
          const ty = y + 5.5;
          const titulo = ev.tipo === 'lista_espera' ? ev.titulo
            : ev.tipo === 'troca' ? `${formatMatricula(ev.titulo)}${ev.matricula_devolver ? ` ↔ ${formatMatricula(ev.matricula_devolver)}` : ''}`
            : formatMatricula(ev.titulo);

          doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(20, 20, 25);
          ...
          doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(60, 60, 70);
          doc.text(format(new Date(ev.data_inicio), ev.dia_todo ? 'dd/MM/yyyy' : 'dd/MM/yy HH:mm', { locale: pt }), COL.data, ty);

          doc.setDrawColor(228, 228, 235); doc.setLineWidth(0.15);
        ...
        doc.text(`Subtotal: ${gestor.eventos.length} evento(s)`, pageW - marginR, y + 5, { align: 'right' });
        y += 7 + 6;
      }
      ...
      let gy = 22;
      doc.setFont('helvetica', 'bold'); doc.setFontSize(14); doc.setTextColor(20, 20, 25);
      ...
      const barH = 10; const barGap = 6;
      const labelW = 48;
      const barStartX = marginL + labelW + 2;

      // Paleta de cores para gestores
      const gestorColors: [number, number, number][] = [
        [99, 102, 241], [34, 197, 94], [249, 115, 22],
        [168, 85, 247], [59, 130, 246], [234, 179, 8],
        [236, 72, 153], [20, 184, 166], [239, 68, 68],
      ];
      ...
      gestores.forEach((g, idx) => {
        ...
        doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(40, 40, 50);
      ...
      // Tabela resumo
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(20, 20, 25);
      doc.text('Resumo', marginL, gy); gy += 6;

      const cGestor = marginL; const cTotal = marginL + 90; const cPct = marginL + 115;

      doc.setFillColor(39, 39, 42);
      doc.rect(marginL - 2, gy - 4, pageW - marginL - marginR + 4, 8, 'F');
      doc.setTextColor(200, 200, 210); doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5);
      ...
      gestores.forEach((g, i) => {
        const pct = eventosFiltrados.length > 0 ? ((g.eventos.length / eventosFiltrados.length) * 100).toFixed(1) : '0.0';
        ...
        doc.setFont('helvetica', 'normal'); doc.setFontSize(8.5); doc.setTextColor(40, 40, 50);
  ...
  const GESTOR_PALETTE = [
    'bg-indigo-500', 'bg-green-500', 'bg-orange-500', 'bg-purple-500',
    'bg-blue-500', 'bg-yellow-500', 'bg-pink-500', 'bg-teal-500', 'bg-red-500',
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-screen h-screen max-w-none max-h-none m-0 rounded-none p-0 flex flex-col gap-0 [&>button]:hidden">
        ...
        <div className="flex items-center justify-between gap-4 border-b px-6 py-3 bg-card shrink-0">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={exportarPDF} disabled={isLoading || exportLoading || eventosFiltrados.length === 0} className="gap-2 h-8 text-xs" size="sm">
              {exportLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileDown className="h-3.5 w-3.5" />}
              PDF Eventos
            </Button>
            <Button variant="outline" onClick={exportarPDFPorGestor} disabled={isLoading || exportGestorLoading || eventosFiltrados.length === 0} className="gap-2 h-8 text-xs" size="sm">
              {exportGestorLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileDown className="h-3.5 w-3.5" />}
              PDF Gestor
            </Button>
            <Button variant="outline" onClick={exportarExcel} disabled={isLoading || exportExcelLoading || eventosFiltrados.length === 0} className="gap-2 h-8 text-xs border-green-600/40 text-green-700 hover:bg-green-600 hover:text-white dark:text-green-400" size="sm">
              {exportExcelLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileSpreadsheet className="h-3.5 w-3.5" />}
              Excel
            </Button>
          ...
          <div className="w-72 shrink-0 border-r bg-muted/20 flex flex-col overflow-y-auto">
            <div className="p-4 space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="space-y-1">
                    ...
                    <input id="relatorio-inicio" type="date" value={dataInicio} onChange={e => setDataInicio(e.target.value)}
                      className="w-full h-8 rounded-md border border-input bg-background px-2 text-xs" />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="relatorio-fim" className="text-xs">Até</Label>
                    <input id="relatorio-fim" type="date" value={dataFim} onChange={e => setDataFim(e.target.value)}
                      className="w-full h-8 rounded-md border border-input bg-background px-2 text-xs" />
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button type="button" onClick={() => setTipoFiltro(null)}
                    className={cn('rounded-full border px-2.5 py-0.5 text-xs font-medium transition-all',
                      tipoFiltro === null ? 'border-foreground bg-foreground text-background' : 'border-border bg-muted/40 text-muted-foreground hover:bg-muted')}>
                  ...
                  {TIPOS_CONFIG.map(t => {
                    ...
                    return (
                      <button key={t.value} type="button" onClick={() => setTipoFiltro(t.value)}
                        className={cn('rounded-full border px-2.5 py-0.5 text-xs font-medium transition-all',
                          tipoFiltro === t.value ? t.colorActive : 'border-border bg-muted/40 text-muted-foreground hover:bg-muted')}>
                        {t.label} ({count})
                      </button>
                    );
                  })}
              ...
              {totalPorTipo.length > 0 && (
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Por Tipo</Label>
                  <div className="space-y-2">
                    {totalPorTipo.map(t => (
                      <div key={t.value} className="space-y-0.5">
                        ...
                        <div className="h-2 rounded-full bg-muted overflow-hidden">
                          <div
                            ...
                            style={{ width: `${(t.count / maxTipoCount) * 100}%`, backgroundColor: `rgb(${TIPO_COLORS_PDF[t.value]?.join(',') || '120,120,120'})` }}
                          />
                        </div>
                      </div>
                    ))}
              ...
              {totalPorGestor.length > 0 && (
                <div className="space-y-2">
                  <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Por Gestor</Label>
                  <div className="space-y-2">
                    {totalPorGestor.map((g, i) => (
                      <div key={g.id} className="space-y-0.5">
                        ...
                        <div className="h-2 rounded-full bg-muted overflow-hidden">
                          <div
                            className={cn('h-full rounded-full transition-all duration-500', GESTOR_PALETTE[i % GESTOR_PALETTE.length])}
              ...
              {isLoading ? (
                ...
                eventosFiltrados.map(ev => {
                  ...
                  const titulo = ev.tipo === 'lista_espera' ? ev.titulo
                    ...
                    : formatMatricula(ev.titulo);
                  return (
                    <div key={ev.id} className={cn('border border-l-4 rounded-lg p-3 text-sm space-y-1 bg-card',
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-semibold truncate">
                          {titulo}{ev.cidade && ` — ${ev.cidade.toUpperCase()}`}
                        </span>
                        {tipoConfig && (
                          <span className={cn('text-[10px] px-2 py-0.5 rounded-full border font-medium shrink-0', tipoConfig.color)}>
                            {tipoConfig.label}
                          </span>
                        )}
                      </div>
                      <div className="text-xs text-muted-foreground flex flex-wrap gap-x-3 gap-y-0.5">
                        <span>{format(new Date(ev.data_inicio), ev.dia_todo ? 'dd/MM/yyyy' : 'dd/MM/yyyy HH:mm', { locale: pt })}</span>
