...
export const TrocaCheckinStep: React.FC<{
  ...
  const {
    tipo, motoristaId, motoristaNome, viaturaAtual, novaViatura,
    userId, data, hora, diaTodo, observacoes, estacaoId, estacaoNome,
  } = trocaData;
  ...
  const makeHandler = (setter: React.Dispatch<React.SetStateAction<SelectedFile[]>>) =>
  ...
  const uploadFiles = async (files: SelectedFile[], contratoId: string, tipoMedia: 'checkin' | 'checkout') => {
    const records: any[] = [];
    for (const { file } of files) {
      const ext = file.name.split('.').pop() || 'bin';
      const path = `${contratoId}/${tipoMedia}/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
      const { error } = await supabase.storage.from('contrato-media').upload(path, file, { contentType: file.type });
      if (error) throw error;
      records.push({ contrato_id: contratoId, tipo: tipoMedia, url: path, nome_ficheiro: file.name, tipo_ficheiro: file.type, tamanho_bytes: file.size, criado_por: userId });
    }
    if (records.length > 0) {
  ...
  const handleConfirm = async () => {
    ...
    const checkinErr = validateCheckinDados(dadosCheckin, viaturaAtual.km_atual ?? 0, viaturaAtual.combustivel ?? '');
    if (checkinErr) { toast.error(`Checkin — ${checkinErr}`); return; }
    const checkoutErr = validateCheckinDados(dadosCheckout, novaViatura.km_atual ?? 0, novaViatura.combustivel ?? '');
    if (checkoutErr) { toast.error(`Checkout — ${checkoutErr}`); return; }
    ...
    try {
      ...
      await supabase.from('viaturas').update({ status: 'disponivel', estacao_id: estacaoId || null }).eq('id', viaturaAtual.id);
      await supabase.from('viaturas').update({ status: 'em_uso', estacao_id: null }).eq('id', novaViatura.id);
      ...
      if (contratoAtual) {
        ...
        await saveCheckinDados({ dados: dadosCheckin, contratoId: contratoAtual.id, viaturaId: viaturaAtual.id, userId, tipo: 'checkin' });
      ...
      await saveCheckinDados({ dados: dadosCheckout, contratoId: newCt.id, viaturaId: novaViatura.id, userId, tipo: 'checkout' });
      ...
      try {
        await supabase.functions.invoke('send-calendar-notification', {
          body: { matricula: eventoPayload.titulo, cidade: estacaoNome, tipo, data_inicio: dataISO, dia_todo: diaTodo },
        });
      } catch { /* non-critical */ }
      ...
      toast.success(`Troca registada — CT-${String(newCt.numero_contrato ?? 0).padStart(4, '0')} criado`);
      setDone(true);
  ...
  const FileGrid = ({ files, onRemove }: { files: SelectedFile[]; onRemove: (id: string) => void }) => (
    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
      {files.map(f => (
        <div key={f.id} className="relative rounded-lg overflow-hidden border border-border aspect-square bg-muted">
          ...
          <button type="button" onClick={() => onRemove(f.id)} className="absolute top-1 right-1 bg-black/60 rounded-full p-0.5 text-white hover:bg-black/80">
            <X className="h-3 w-3" />
          </button>
        </div>
      ))}
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-3 bg-card shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">
            {motoristaNome} — {formatMatricula(viaturaAtual.matricula)} → {formatMatricula(novaViatura.matricula)}
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="rounded-lg border border-blue-200 dark:border-blue-800 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-blue-700 dark:text-blue-300">Checkin — Viatura Devolvida</p>
                <p className="text-xs text-muted-foreground">{viaturaAtual.marca} {viaturaAtual.modelo}</p>
              </div>
              <Badge variant="outline" className="font-mono shrink-0">{formatMatricula(viaturaAtual.matricula)}</Badge>
            </div>

            {contratoAtual && (
              <p className="text-xs text-muted-foreground">
                Contrato CT-{String(contratoAtual.numero_contrato ?? 0).padStart(4, '0')} será encerrado.
              </p>
            )}
            ...
            <input ref={fileInputCheckinRef} type="file" accept="image/*,video/*" multiple className="hidden" onChange={makeHandler(setFilesCheckin)} />
            <button
              type="button"
            ...
            >
              ...
              <span>Fotos / vídeos de checkin <span className="text-xs text-destructive font-medium">*obrigatório</span></span>
            </button>
            {filesCheckin.length > 0 && <FileGrid files={filesCheckin} onRemove={(id) => removeFile(setFilesCheckin, id)} />}
          </div>
          ...
          <div className="rounded-lg border border-green-200 dark:border-green-800 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-green-700 dark:text-green-300">Checkout — Nova Viatura</p>
                <p className="text-xs text-muted-foreground">{novaViatura.marca} {novaViatura.modelo}</p>
              </div>
              <Badge variant="outline" className="font-mono shrink-0">{formatMatricula(novaViatura.matricula)}</Badge>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={cidadeAssinatura} onChange={e => setCidadeAssinatura(e.target.value)} className="h-8 text-sm" placeholder="Ex: Leiria" />
              </div>
            </div>
            ...
            <input ref={fileInputCheckoutRef} type="file" accept="image/*,video/*" multiple className="hidden" onChange={makeHandler(setFilesCheckout)} />
            <button
              type="button"
            ...
            >
              <Upload className="h-6 w-6 opacity-40" />
              <span>Fotos / vídeos de checkout <span className="text-xs text-destructive font-medium">*obrigatório</span></span>
            </button>
            {filesCheckout.length > 0 && <FileGrid files={filesCheckout} onRemove={(id) => removeFile(setFilesCheckout, id)} />}
          </div>
