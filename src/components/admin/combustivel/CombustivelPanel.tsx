    ...
    <Card className="border-border/50 bg-card/50">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Gestão de Cartões Frota
        </CardTitle>
        <CardDescription>
          Configure as credenciais para importação automática e gestão de consumos.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-3 mb-6">
            <TabsTrigger value="bp" className="gap-2">
              BP Fleet{' '}
              {configs.bp?.ativo && <Badge className="bg-green-500 h-2 w-2 p-0 rounded-full" />}
            </TabsTrigger>
            <TabsTrigger value="repsol" className="gap-2">
              Repsol Solred{' '}
              {configs.repsol?.ativo && <Badge className="bg-green-500 h-2 w-2 p-0 rounded-full" />}
            </TabsTrigger>
            <TabsTrigger value="edp" className="gap-2">
              EDP Mobilidade{' '}
              {configs.edp?.ativo && <Badge className="bg-green-500 h-2 w-2 p-0 rounded-full" />}
            </TabsTrigger>
          </TabsList>
            ...
            <TabsContent key={tab} value={tab} className="space-y-6">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder={`ID da conta ${tab.toUpperCase()}`}
                  />
                </div>
                <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                <div className="text-sm text-muted-foreground">
                  {configs[tab]?.ultimo_sync &&
                    `Último sync: ${new Date(configs[tab].ultimo_sync!).toLocaleString('pt-PT')}`}
                </div>
              ...
              <div className="p-4 bg-primary/5 rounded-lg border border-primary/10">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Após configurar as credenciais, o sistema fará o match automático das faturas{' '}
                  {tab.toUpperCase()} com os motoristas baseando-se no número do cartão associado na
                  ficha de cada motorista. Use o botão de importação no separador Administrativo
                  para carregar os ficheiros CSV.
                </p>
              </div>
            </TabsContent>
          ))}
