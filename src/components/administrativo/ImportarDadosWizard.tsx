...
type Plataforma = 'bolt' | 'uber' | 'bp' | 'repsol' | 'edp';
...
export const ImportarDadosWizard: React.FC<ImportarDadosWizardProps> = ({
    ...
    const verificar = async () => {
      ...
      try {
        ...
        if (plataforma === 'bolt') {
          ...
          // bp / repsol / edp
          const table = `${plataforma}_transacoes` as
            | 'bp_transacoes'
            | 'repsol_transacoes'
            | 'edp_transacoes';
          const { count } = await supabase
  ...
  const handleImportar = async () => {
    ...
    try {
      ...
      const imp = data.imported ?? data.processados ?? data.total_imported ?? 0;
          ...
          >
            <span
              className={cn(
                ...
                selecionado ? cn(p.accent, 'text-white') : cn(p.bg, p.text)
              )}
            >
