...
import { Loader2, Search, Fuel, Euro, X, Upload, CheckCircle2, MapPin, Calendar as CalendarIcon, Zap } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Button variant="outline" size="sm" onClick={() => setShowImportDialog(true)}><Upload className="h-4 w-4 mr-2" />Importar EDP</Button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Carregamentos" value={stats.total} icon={<Zap className="h-4 w-4" />} />
        <StatCard title="Total Energia" value={`${stats.energia.toFixed(2)} kWh`} icon={<Zap className="h-4 w-4" />} />
        <StatCard title="Custo Total" value={`€${stats.valor.toFixed(2)}`} icon={<Euro className="h-4 w-4" />} color="text-green-500" />
      </div>

      <div className="border rounded-lg overflow-hidden">
        {loading ? <div className="p-12 text-center"><Loader2 className="animate-spin mx-auto" /></div> : 
        <Table>
          <TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Motorista</TableHead><TableHead>Cartão</TableHead><TableHead>Ponto de Carga</TableHead><TableHead className="text-right">Energia</TableHead><TableHead className="text-right">Valor</TableHead></TableRow></TableHeader>
          <TableBody>
            {filtered.map(t => (
              <TableRow key={t.id}>
                ...
                <TableCell className="text-right font-medium">{t.quantity?.toFixed(2)} kWh</TableCell>
                <TableCell className="text-right font-bold text-green-500">€{t.amount?.toFixed(2)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>}
