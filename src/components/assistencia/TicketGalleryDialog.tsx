...
import type { TicketAnexo as Anexo } from '@/types/ticket';

interface TicketGalleryDialogProps {
  open: boolean;
...
export function TicketGalleryDialog({
  open,
  onOpenChange,
  anexos,
  ticketNumero,
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Galeria de Multimédia - Ticket #{String(ticketNumero).padStart(4, '0')}
          </DialogTitle>
          <DialogDescription>
                ...
                {checkinAnexos.length === 0 ? (
                  ...
                  checkinAnexos.map(anexo => (
                    <MediaThumb
                      key={anexo.id}
                      anexo={anexo}
                      onClick={() => onOpenLightbox(anexos, anexos.indexOf(anexo))}
                    />
                  ))
                )}
              </div>
