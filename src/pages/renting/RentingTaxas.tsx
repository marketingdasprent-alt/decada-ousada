...
const EMPTY_FORM = { nome: '', descricao: '', tipo_valor: 'percentagem' as 'percentagem' | 'fixo', percentagem: '', valor_fixo: '', aplicar_automaticamente: false, ativa: true };
...
const RentingTaxas = () => {
  ...
  const filtered = taxas.filter((t) => !search || t.nome.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleSave = async () => {
    const valorPreenchido = form.tipo_valor === 'percentagem' ? form.percentagem.trim() : form.valor_fixo.trim();
    ...
    try {
      ...
      if (editing) {
        ...
        const { error } = await supabase.from('renting_taxas').insert({ ...payload, org_id: orgId });
        if (error) throw error;
  ...
  return (
    <div className="w-full">
      <StickyPageHeader
        title="Taxas"
        description={isLoading ? 'A carregar...' : `${filtered.length} taxa${filtered.length !== 1 ? 's' : ''}`}
        icon={Percent}
      >
        <Button onClick={openNew} size="sm"><Plus className="h-4 w-4 mr-2" />Nova Taxa</Button>
      </StickyPageHeader>
      ...
      {isLoading ? (
        ...
        <div className="border rounded-lg flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">{search ? 'Nenhuma taxa encontrada' : 'Ainda não há taxas criadas'}</p>
          {!search && <Button onClick={openNew} variant="outline" size="sm"><Plus className="h-4 w-4 mr-2" />Criar primeira taxa</Button>}
        </div>
      ) : (
              ...
              {filtered.map((t) => (
                <TableRow key={t.id} className="hover:bg-muted/50">
                  <TableCell className="font-medium">
                    {t.nome}
                    {t.descricao && <span className="block text-xs text-muted-foreground">{t.descricao}</span>}
                  </TableCell>
                  ...
                  <TableCell><Badge variant={t.ativa ? 'default' : 'secondary'} className="text-xs">{t.ativa ? 'Activa' : 'Inactiva'}</Badge></TableCell>
                  <TableCell className="py-2">
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(t)}><Pencil className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setDeleteTarget(t)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) setDialogOpen(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editing ? 'Editar Taxa' : 'Nova Taxa'}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>Nome *</Label>
              <Input value={form.nome} onChange={(e) => setForm((p) => ({ ...p, nome: e.target.value }))} placeholder="Ex: IVA 23%, Taxa Aeroporto" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={form.tipo_valor} onValueChange={(v: 'percentagem' | 'fixo') => setForm((p) => ({ ...p, tipo_valor: v }))} className="flex gap-6">
              ...
              {form.tipo_valor === 'percentagem' ? (
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input type="number" min="0" max="100" step="0.01" value={form.percentagem} onChange={(e) => setForm((p) => ({ ...p, percentagem: e.target.value }))} placeholder="Ex: 23" className="pr-8" />
                ...
                <div className="space-y-2">
                  <Label>Valor (€) *</Label>
                  <Input type="number" min="0" step="0.01" value={form.valor_fixo} onChange={(e) => setForm((p) => ({ ...p, valor_fixo: e.target.value }))} placeholder="0.00" />
                </div>
              )}
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Adicionada por defeito a novas reservas</p>
              </div>
              <Switch checked={form.aplicar_automaticamente} onCheckedChange={(v) => setForm((p) => ({ ...p, aplicar_automaticamente: v }))} />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <Label>Activa</Label>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>A taxa <strong>{deleteTarget?.nome}</strong> será eliminada permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
