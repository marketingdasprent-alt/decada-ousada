...
const ViaturaCombustiveis = () => {
  ...
  const filtered = combustiveis.filter((c) => !search || c.nome.toLowerCase().includes(search.toLowerCase()));

  const openNew = () => { setEditing(null); setForm(EMPTY_FORM); setDialogOpen(true); };
  const openEdit = (c: Combustivel) => { setEditing(c); setForm({ nome: c.nome, ativo: c.ativo }); setDialogOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        const { error } = await supabase.from('viatura_combustiveis').update(payload).eq('id', editing.id);
        if (error) throw error;
        toast({ title: 'Combustível actualizado' });
      } else {
        const { error } = await supabase.from('viatura_combustiveis').insert({ ...payload, org_id: orgId });
        if (error) throw error;
  ...
  return (
    <div className="w-full">
      <StickyPageHeader
        title="Combustíveis"
        description={isLoading ? 'A carregar...' : `${filtered.length} combustível${filtered.length !== 1 ? 'is' : ''}`}
        icon={Fuel}
      >
        <Button onClick={openNew} size="sm"><Plus className="h-4 w-4 mr-2" />Novo Combustível</Button>
      </StickyPageHeader>
      ...
      {isLoading ? (
        ...
        <div className="border rounded-lg flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">{search ? 'Nenhum combustível encontrado' : 'Ainda não há combustíveis criados'}</p>
          {!search && <Button onClick={openNew} variant="outline" size="sm"><Plus className="h-4 w-4 mr-2" />Criar primeiro combustível</Button>}
        </div>
      ) : (
            ...
            <TableBody>
              {filtered.map((c) => (
                <TableRow key={c.id} className="hover:bg-muted/50">
                  ...
                  <TableCell><Badge variant={c.ativo ? 'default' : 'secondary'} className="text-xs">{c.ativo ? 'Activo' : 'Inactivo'}</Badge></TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) setDialogOpen(false); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{editing ? 'Editar Combustível' : 'Novo Combustível'}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>Nome *</Label>
              <Input value={form.nome} onChange={(e) => setForm((p) => ({ ...p, nome: e.target.value }))} placeholder="Ex: Elétrico, Gasolina, Diesel" />
            </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>O combustível <strong>{deleteTarget?.nome}</strong> será eliminado permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
