...
const ViaturaTipos = () => {
  ...
  const openEdit = (t: ViatTipo) => { setEditing(t); setForm({ nome: t.nome, ativo: t.ativo }); setDialogOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        const { error } = await supabase.from('viatura_tipos').insert({ ...payload, org_id: orgId });
        if (error) throw error;
  ...
  return (
    <div className="w-full">
      <StickyPageHeader
        ...
        description={isLoading ? 'A carregar...' : `${filtered.length} tipo${filtered.length !== 1 ? 's' : ''}`}
        icon={Tag}
      >
        <Button onClick={openNew} size="sm"><Plus className="h-4 w-4 mr-2" />Novo Tipo</Button>
      </StickyPageHeader>
      ...
      {isLoading ? (
        ...
        <div className="border rounded-lg flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">{search ? 'Nenhum tipo encontrado' : 'Ainda não há tipos criados'}</p>
          {!search && <Button onClick={openNew} variant="outline" size="sm"><Plus className="h-4 w-4 mr-2" />Criar primeiro tipo</Button>}
        </div>
      ) : (
            ...
            <TableBody>
              {filtered.map((t) => (
                <TableRow key={t.id} className="hover:bg-muted/50">
                  ...
                  <TableCell><Badge variant={t.ativo ? 'default' : 'secondary'} className="text-xs">{t.ativo ? 'Activo' : 'Inactivo'}</Badge></TableCell>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>Nome *</Label>
              <Input value={form.nome} onChange={(e) => setForm((p) => ({ ...p, nome: e.target.value }))} placeholder="Ex: Ligeira TVDE, Comercial, Ligeira RENT" />
            </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>O tipo <strong>{deleteTarget?.nome}</strong> será eliminado permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
